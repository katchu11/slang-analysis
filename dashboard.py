...
st.set_page_config(
    page_title="WTAC - What's That Acronym, Claude?",
    page_icon="🤔",
    layout="wide"
)
...
def main():
    ...
    with st.sidebar:
        ...
        slang_term = st.text_input(
            "Enter slang term:",
            value="YOLO",
            placeholder="e.g., YOLO, FOMO, SMH"
        ).upper().strip()
...
def run_analysis_with_progress(slang_term: str, sample_size: int, api_key: str):
    ...
    async def run_with_progress():
        ...
        try:
            ...
            haiku_status.text("🎭 Claude 3.5 Haiku generating creative interpretations...")
            ...
            sonnet_status.text("🧠 Claude 4 Sonnet parsing interpretations into structured data...")
...
def display_results(slang_term: str, results: Dict[str, Any]):
    ...
    valid_results = [r for r in parsed_results if r.get('primary_meaning') and 'error' not in r]
